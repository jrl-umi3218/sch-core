...
public:
  Depth_Edge() : m_triangle(0x0), m_index(0) {}
  Depth_Edge(Depth_Triangle * triangle, int index) : m_triangle(triangle), m_index(index) {}
...
public:
  ...
  SCH_API Depth_Triangle(Index_t i0, Index_t i1, Index_t i2) : m_obsolete(false)
  {
    m_indices[0] = i0;
    m_indices[1] = i1;
    m_indices[2] = i2;
  ...
  {
    return m_lambda1 >= Scalar(0.0) && m_lambda2 >= Scalar(0.0) && m_lambda1 + m_lambda2 <= m_det;
  }
  ...
  SCH_API Vector3 getClosestPoint(const Vector3 * points) const
  {
    ...
    return p0 + ((points[m_indices[1]] - p0) * m_lambda1 + (points[m_indices[2]] - p0) * m_lambda2) / m_det;
  }
...
public:
  SCH_API Depth_TriangleStore() : m_free(0) {}
