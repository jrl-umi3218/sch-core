...
{
  for(unsigned i = 0; i < objectVector.size(); ++i) delete objectVector[i];
  objectVector.clear();
  for(unsigned i = 0; i < stpObjects.size(); ++i) delete stpObjects[i];
  stpObjects.resize(0);
...
void TestMaterial::RandomTestSupportFunction()
{
  ...
  for(long j = 0; j < RandomTestEnd; j++)
  {
    ...
    Vector3 v(sqrt(-2 * log(a)) * cos(2 * pi * b), sqrt(-2 * log(b)) * cos(2 * pi * a),
              sqrt(-2 * log(c)) * cos(2 * pi * d));
  ...
  for(long j = 0; j < RandomTestEnd; j++) outfile << supportVector[j] << std::endl;
  outfile.close();
#endif // OUTPUT_FILE
}
...
#ifdef DO_TEST
    for(size_t i = 0; i < sObj.size(); i++) supportVector[j * sObj.size() + i] = sObj[i]->support(v);
#endif
  }
...
#  ifdef OUTPUT_FILE
  ...
  outfile.precision(18);
  for(size_t i = 0; i < supportVector.size(); ++i) outfile << supportVector[i] << std::endl;
  outfile.close();
#  endif
#endif
}
...
void TestMaterial::initializeUniverse()
{
  ...
  for(unsigned i = 0; i < objectVector.size(); ++i) sObj.addObject(objectVector[i]);
#endif
  ...
  {
    Vector3 position((1. + 7 * i % 5 - 3.), (5 * i % 6 - 3.) * (5.0 / 6.), (5 * i % 7 - 3.) * (5.0 / 7.));
...
void TestMaterial::TestPrecision()
{
  ...
  for(long i = 0; i < AnimationEnd; i++)
  {
    ...
    */
    ...
    for(size_t j = 0; j < sObj.size(); j++) sObj[j]->addRotation(angle, axe);
...
#  endif
  ...
  std::cout << "  computation time: " << ((double)(end - begin) / CLOCKS_PER_SEC) << " : "
            << ((double)(end - begin) / CLOCKS_PER_SEC) / (AnimationEnd) << std::endl;
...
void TestMaterial::TestAnimation()
{
  ...
  for(long i = 0; i < AnimationEnd; i++)
  {
    for(size_t j = 0; j < sObj.size(); j++)
    {
      ...
      sObj[j]->setPosition(position
                           + Vector3(sin((20 * (j % 3 + 1) + 2 * j) * sin(0.2 * AnimationSpeed * i)),
                                     sin((71 - 140 * (j % 2 - 1) + 2 * j) * sin(0.15 * AnimationSpeed * i)),
                                     sin((20 + ((j * 5) % 7) * 5 + 3 * j) * sin(0.2 * AnimationSpeed * i)))
                                 * AnimationScale);
    }
...
void TestMaterial::GeneralTest()
{
  ...
  for(size_t k = 0; k < stppObjects.size(); k++)
  {
    ...
    Vector3 position((1. + 7 % 5 - 3.), ((5 % 6 - 3) * (5.0 / 6)), ((5 % 7 - 3) * (5.0 / 7)));
    position *= DispersionScale;
...
#  endif // OUTPUT_FILE
    ...
    for(long i = 0; i < AnimationEnd; i++)
    {
      ...
      testscene[1]->setPosition(position
                                + Vector3(sin((20 * (1)) * sin(0.2 * AnimationSpeed * i)),
                                          sin((71 - 140 * (1)) * sin(0.15 * AnimationSpeed * i)),
                                          sin((20) * sin(0.2 * AnimationSpeed * i)))
                                      * AnimationScale);
    }
    ...
    outfile << ((double)(end - begin - lostTime) / CLOCKS_PER_SEC) << " "
            << ((double)(end - begin - lostTime) / CLOCKS_PER_SEC) / (AnimationEnd) << std::endl;
    std::cout << ((double)(end - begin - lostTime) / CLOCKS_PER_SEC) << " "
              << ((double)(end - begin - lostTime) / CLOCKS_PER_SEC) / (AnimationEnd) << std::endl;
  }
  outfile.close();
}
