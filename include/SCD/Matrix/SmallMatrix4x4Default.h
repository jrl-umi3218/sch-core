...
namespace MAL_Default
{
  ...
  template <typename T> struct Matrix4x4D
    {
      /*! The data array. */
      T m[16];
	 ...
	 m[8]  = 0.0;  m[9]  = 0.0; m[10] = 0.0; m[11] = 0.0;
      ...
      Matrix4x4D<T> (const T x)
      {
	for(int i=0;i<16;m[i++]=x);
      }
	
      /*! Copy constructor */
      Matrix4x4D<T> (const struct Matrix4x4D<T> &v)
      {
	for(int i=0;i<16;i++)
	  m[i] = v.m[i];
      }
      ...
      Matrix4x4D<T> (const struct Matrix4x4D<T2> &v)
      {
	for(int i=0;i<16;i++)
	  m[i] = v.m[i];
      }
	...
	m[0] = m[5] = m[10] = m[15] = 1.0;
      ...
      /*! Addition operator */
      Matrix4x4D<T>  operator+(const Matrix4x4D<T> & B)
      {
	Matrix4x4D<T> A;
	...
	return A;
      ...
      struct Matrix4x4D<T>  operator-(const struct Matrix4x4D<T> &B)
      {
	Matrix4x4D<T> A;
	A.m[0] = m[0] - B.m[0]; A.m[1] = m[1] - B.m[1]; A.m[2] = m[2] - B.m[2];
	A.m[3] = m[3] - B.m[3]; A.m[4] = m[4] - B.m[4]; A.m[5] = m[5] - B.m[5];
	...
	return A;
      ...
      Matrix4x4D<T>  operator* (const Matrix4x4D<T> &B) const
      {
	Matrix4x4D<T> A;
	...
	return A;
      ...
      /*! Multiplication operator with another matrix */
      void  CeqthismulB (const Matrix4x4D<T> &B, Matrix4x4D<T> &C) const
      {
