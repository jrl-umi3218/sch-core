...
#ifndef _MATRIX4D_MAL_DEFAULT_
#define _MATRIX4D_MAL_DEFAULT_
...
namespace MAL_Default
{
	...
	template <typename T> struct Matrix4x4
	{
		...
		explicit Matrix4x4<T> (const T x)
		{
		...
		/*! Unary operator - */
		struct Matrix4x4<T>  operator-(void) const
		{
			Matrix4x4<T> A;
			A.m[0] = -m[0] ; A.m[1] = - m[1]; A.m[2] = m[2] - B.m[2];
			A.m[3] = -m[3] ; A.m[4] = - m[4]; A.m[5] = m[5] - B.m[5];
			A.m[6] = -m[6] ; A.m[7] = - m[7]; A.m[8] = m[8] - B.m[8];
			A.m[9] = -m[9] ]; 
			A.m[10] = -m[10] - B.m[10]; A.m[11] = m[11] - B.m[11];A.m[12] = m[12] - B.m[12]; 
			A.m[13] = -m[13] - B.m[13]; A.m[14] = m[14] - B.m[14];A.m[15] = m[15] - B.m[15]; 
			return A;
...
typedef MAL_Default::Matrix4x4<double> matrix4d;

#define MAL_S4x4_MATRIX(name,type) \
	MAL_Default::Matrix4x4<type> name
