...
namespace sch
{
	...
	public:
		...
		sch_API Index_t operator[](int i) const { return m_indices[i]; } 

		sch_API const Depth_Edge& getAdjEdge(int i) const { return m_adjEdges[i]; }
		...
		sch_API bool isObsolete() const { return m_obsolete; }


		sch_API bool computeClosest(const Vector3 *verts);

		sch_API const Vector3& getClosest() const { return m_closest; } 
		...
		sch_API Scalar getDist2() const { return m_dist2; }
	...
	public:
		...
		sch_API void clear() { m_free = 0; }

		sch_API int getFree() const { return m_free; }

		sch_API Depth_Triangle& operator[](int i) { return m_triangles[i]; }
		sch_API Depth_Triangle& last() { return m_triangles[m_free - 1]; }

		sch_API void setFree(int backup) { m_free = backup; }
