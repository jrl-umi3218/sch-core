...
private:
  ...
  // static one
  explicit oserializer() : basic_oserializer(*boost::serialization::type_info_implementation<T>::type::get_instance())
  {
  }
...
public:
  ...
  {
    typedef BOOST_DEDUCED_TYPENAME boost::serialization::type_info_implementation<T>::type::is_polymorphic::type typex;
    return typex::value;
  }
...
template<class Archive, class T>
BOOST_DLLEXPORT void oserializer<Archive, T>::save_object_data(basic_oarchive & ar, const void * x) const
{
  // make sure call is routed through the highest interface that might
  // be specialized by the user.
  boost::serialization::serialize_adl(boost::smart_cast_reference<Archive &>(ar),
...
private:
  ...
  virtual BOOST_DLLEXPORT void save_object_ptr(basic_oarchive & ar, const void * x) const BOOST_USED;
...
template<class T, class Archive>
BOOST_DLLEXPORT const pointer_oserializer<T, Archive> & pointer_oserializer<T, Archive>::instantiate()
{
  return instance;
}
...
template<class T, class Archive>
BOOST_DLLEXPORT void pointer_oserializer<T, Archive>::save_object_ptr(basic_oarchive & ar, const void * x) const
{
  assert(NULL != x);
  // make sure call is routed through the highest interface that might
  // be specialized by the user.
...
: archive_pointer_oserializer<Archive>(*boost::serialization::type_info_implementation<T>::type::get_instance()),
...
struct save_non_pointer_type
{
  ...
  typedef BOOST_DEDUCED_TYPENAME mpl::eval_if<
      // if its primitive
      mpl::equal_to<boost::serialization::implementation_level<T>, mpl::int_<boost::serialization::primitive_type>>,
      mpl::identity<save_primitive>,
      // else
      BOOST_DEDUCED_TYPENAME mpl::eval_if<
          // class info / version
          mpl::greater_equal<boost::serialization::implementation_level<T>,
          ...
          BOOST_DEDUCED_TYPENAME mpl::eval_if<
              // no tracking
              mpl::equal_to<boost::serialization::tracking_level<T>, mpl::int_<boost::serialization::track_never>>,
              // do a fast save
  ...
  static void invoke(Archive & ar, const T & t)
  {
    ...
    BOOST_STATIC_ASSERT((mpl::greater_equal<boost::serialization::implementation_level<T>,
                                            mpl::int_<boost::serialization::primitive_type>>::value));
    typex::invoke(ar, t);
  };
};
...
struct save_pointer_type
{
  ...
  struct abstract
  {
    static const basic_pointer_oserializer * register_type(Archive & /* ar */)
    {
      ...
      BOOST_STATIC_ASSERT(boost::serialization::type_info_implementation<T>::type::is_polymorphic::value);
  ...
  static const basic_pointer_oserializer * register_type(Archive & ar, T & /*t*/)
  {
    ...
    typedef BOOST_DEDUCED_TYPENAME mpl::eval_if<serialization::is_abstract<T>, mpl::identity<abstract<T>>,
                                                mpl::identity<non_abstract<T>>>::type typex;
  ...
  struct polymorphic
  {
    static void save(Archive & ar, const T & t, const basic_pointer_oserializer * bpos_ptr)
    {
      ...
      if(NULL == vp)
      {
        boost::throw_exception(archive_exception(archive_exception::unregistered_cast));
      }
      ...
      if(NULL == bpos_ptr) boost::throw_exception(archive_exception(archive_exception::unregistered_class));
      ar.save_pointer(vp, bpos_ptr);
    }
  };
  ...
  static void save(Archive & ar, const T & t, const basic_pointer_oserializer * bpos_ptr)
  {
    typedef BOOST_DEDUCED_TYPENAME
        mpl::eval_if<BOOST_DEDUCED_TYPENAME boost::serialization::type_info_implementation<T>::type::is_polymorphic,
                     mpl::identity<polymorphic<T>>, mpl::identity<non_polymorphic<T>>>::type typey;
...
struct save_array_type
{
  static void invoke(Archive & ar, const T & t)
  {
    ...
    int i;
    for(i = 0; i < count; ++i) ar << boost::serialization::make_nvp("item", t[i]);
  }
};
...
template<class Archive, class T>
BOOST_DLLEXPORT const basic_pointer_oserializer & instantiate_pointer_oserializer(Archive * /* ar = NULL */,
                                                                                  T * /* t = NULL */
                                                                                  ) BOOST_USED;
// definition
template<class Archive, class T>
...
{
  typedef BOOST_DEDUCED_TYPENAME mpl::eval_if<
      is_pointer<T>, mpl::identity<detail::save_pointer_type<Archive, T>>,
      // else
      BOOST_DEDUCED_TYPENAME mpl::eval_if<
          is_enum<T>, mpl::identity<detail::save_enum_type<Archive, T>>,
          // else
          BOOST_DEDUCED_TYPENAME mpl::eval_if<is_array<T>, mpl::identity<detail::save_array_type<Archive, T>>,
                                              // else
                                              mpl::identity<detail::save_non_pointer_type<Archive, T>>>>>::type typex;
  typex::invoke(ar, t);
}
...
struct check_tracking
{
  typedef BOOST_DEDUCED_TYPENAME mpl::if_<
      ...
      BOOST_DEDUCED_TYPENAME mpl::equal_to<serialization::tracking_level<T>, mpl::int_<serialization::track_never>>,
