...
public:
  ...
  /*! copy constructor */
  template<bool b>
  inline Vector3T(const struct Vector3T<T, b> & v) : m_x(v.m_x), m_y(v.m_y), m_z(v.m_z)
  {
  }
  ...
  inline Vector3T operator*(const Matrix3x3T<T> & m) const
  {
    ...
    Vector3T vr(m_x * m[0] + m_y * m[3] + m_z * m[6], m_x * m[1] + m_y * m[4] + m_z * m[7],
  ...
  inline T optimizedNorm()
  {
    if(normOptimization)
    {
      if(Norm<T, normOptimization>::normOutdated()) setNorm(static_cast<T>(sqrt(m_x * m_x + m_y * m_y + m_z * m_z)));
